...
def main():
    ...
    for idx, task_id in enumerate(task_id_ls):  # task_id is the actual id of the task. idx is just the index.
        ...
        with Timer() as t:
            ...
            if "R3M" in args.model_path_folder:
                with open(os.path.join(args.model_path_folder, f"eval_tasks_on_ori_envs_R3M_seed{args.seed}",
                                       f"succ_list_evaluation_on_ori_envs_benchmark_{args.benchmark}.npy"),
                          'wb') as f:
                    np.save(f, np.array(succ_list))
            elif "LIV" in args.model_path_folder:
                with open(os.path.join(args.model_path_folder, f"eval_tasks_on_ori_envs_LIV_seed{args.seed}",
                                       f"succ_list_evaluation_on_ori_envs_benchmark_{args.benchmark}.npy"),
                          'wb') as f:
                    np.save(f, np.array(succ_list))
            else:
                with open(os.path.join(args.model_path_folder, f"eval_tasks_on_ori_envs_seed{args.seed}",
                                       f"succ_list_evaluation_on_ori_envs_benchmark_{args.benchmark}.npy"),
                          'wb') as f:
                    np.save(f, np.array(succ_list))
        ...
        if "R3M" in args.model_path_folder:
            with open(os.path.join(args.model_path_folder, f"eval_tasks_on_ori_envs_R3M_seed{args.seed}",
                                   f"succ_list_evaluation_on_ori_envs_benchmark_{args.benchmark}.npy"), 'wb') as f:
                np.save(f, np.array(succ_list))
        elif "LIV" in args.model_path_folder:
            with open(os.path.join(args.model_path_folder, f"eval_tasks_on_ori_envs_LIV_seed{args.seed}",
                                   f"succ_list_evaluation_on_ori_envs_benchmark_{args.benchmark}.npy"), 'wb') as f:
