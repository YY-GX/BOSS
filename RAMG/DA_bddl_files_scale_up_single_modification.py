...
for seed in seed_ls:
    ...
    dst_bddl_folder_new = Path(dst_bddl_folder) / f"data_augmentation"
    ...
    for i, bddl_file in enumerate(sorted(bddl_files)):
        ...
        while cnt < num_diff_combination:
            if dead_loop_cnt > 10000:
                print(f"[ERROR] Dead loop for bddl file: {bddl_file}; Only {cnt}/{num_diff_combination} "
                      f"combinations obtained :(")
